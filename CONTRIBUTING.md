# Contributing to mcp-einvoicing-de
...
```bash
uv run ruff check src/mcp_einvoicing_de/ tests/ audit/
uv run ruff format --check src/mcp_einvoicing_de/ tests/ audit/
uv run mypy src
```
...
```bash
uv run ruff check --fix src/mcp_einvoicing_de/ tests/ audit/
uv run ruff format src/mcp_einvoicing_de/ tests/ audit/
```
...
- [ ] Validation fixes reference the relevant rule ID (e.g. `BR-DE-15`)
...
Please open an issue at https://github.com/cmendezs/mcp-einvoicing-de/issues and include:
